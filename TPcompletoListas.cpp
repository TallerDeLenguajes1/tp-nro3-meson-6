...
Nodo * Crear_Nodo();
void Imprimir_lista(Nodo *Start);
void Insertar_inicio(Nodo **Start);
void Insertar_final(Nodo **Start);
void Existe_elemento(Nodo *Start,int id);
void Eliminar_elemento(Nodo *nodo);
void Eliminar_elemento_especifico(Nodo *Start,int id);
//Main
int main(void){
	srand(time(NULL));//Randomiza la seed
	//Inicio la lista
	Nodo *Start = NULL;
	Insertar_inicio(&Start);
	Insertar_final(&Start);
	Insertar_final(&Start);
	Imprimir_lista(Start);
	Eliminar_elemento_especifico(Start,2);
	printf("\nSe elimino 2\n");
	Imprimir_lista(Start);
...
Nodo * Crear_Nodo(){
	...
	//Pregunto si el nodo es distinto del nulo
	if (nuevo != NULL)
	{
		nuevo->personaje = (PERSONAJE*) malloc(sizeof(PERSONAJE));
		nuevo->personaje->DatosPersonales = (DATOS*) malloc(sizeof(DATOS));
		nuevo->personaje->Caracteristicas = (CARACTERISTICAS*) malloc(sizeof(CARACTERISTICAS));
		cargar_datos(nuevo->personaje->DatosPersonales);
		cargarcaract(nuevo->personaje->Caracteristicas);
...
void Imprimir_lista(Nodo *Start){
	Nodo * Aux = Start;
	while(Aux != NULL){
		mostrar_datos(Aux->personaje->DatosPersonales);
		mostrarcaract(Aux->personaje->Caracteristicas);
		Aux = Aux->siguiente;
	}
}
void Insertar_inicio(Nodo **Start){
...
void Existe_elemento(Nodo *Start,int id){
	Nodo *Aux = Start;
	while(Aux != NULL){
		if (Aux->personaje->DatosPersonales->id == id)
		{
			printf("El personaje que busca está en la lista.\n");
			return;
		}
		Aux = Aux->siguiente;
	}
	printf("El personaje no está en la lista.\n");
}
void Eliminar_elemento(Nodo *nodo){
	if (nodo != NULL)
	{
...
void Eliminar_elemento_especifico(Nodo *Start,int id){
	Nodo * Aux = Start;
	Nodo * Anterior;
	while(Aux != NULL && Aux->personaje->DatosPersonales->id != id){
		Anterior = Aux;
		Aux = Aux->siguiente;
	}
	if (Aux != NULL)
